...
function Line({
  ...
  return (
    ...
    >
      <p className="h-[8px] text-xs text-gray-400">{value.toFixed(0)}h</p>
      <div className="ml-[5%] w-[95%] h-px bg-gray-400"></div>
    </div>
  );
}
export function WeeklyDiagramm({ data }: { data: WeeklyData; }) {
  ...
  return (
    <>
      ...
      <Line y={50} height={HEIGHT} offset={OFFSET} value={max / 60 / 2} />
